...
list_of_commands = '["circle", "square", "triangle", "red", "green", "blue", "clear", "reset", "bigger", "smaller", "left", "right", "up", "down", "three d", "two d", "rotate", "[unk]"]'
        ...
        while True:
            ...
            while not audio_queue.empty():
                ...
                if rec.AcceptWaveform(data):
                    ...
                    if command:
                        if command in ["circle", "square", "triangle"]: shape.type = command
                        elif "three d" in command: shape.is_3d = True
                        elif "two d" in command: shape.is_3d = False
                        elif "rotate" in command: shape.is_rotating = not shape.is_rotating
                        elif "bigger" in command: shape.size = min(shape.size + 40, 400)
                        elif "smaller" in command: shape.size = max(shape.size - 40, 30)
                        elif "left" in command:  shape.x = max(shape.x - shape.step, 0)
                        elif "right" in command: shape.x = min(shape.x + shape.step, W)
                        elif "up" in command:    shape.y = max(shape.y - shape.step, 0)
                        elif "down" in command:  shape.y = min(shape.y + shape.step, H)
                        elif "red" in command: shape.color = (0,0,255)
                        elif "green" in command: shape.color = (0,255,0)
                        elif "blue" in command: shape.color = (255,0,0)
                        elif "clear" in command or "reset" in command: shape.reset()
            ...
            if shape.type:
                if not shape.is_3d:
                    if shape.type == "circle":
                        cv2.circle(frame, (cx, cy), s, shape.color, 5)
                    elif shape.type == "square":
                        cv2.rectangle(frame, (cx-s, cy-s), (cx+s, cy+s), shape.color, 5)
                    elif shape.type == "triangle":
                        pts = np.array([[cx, cy-s], [cx-s, cy+s], [cx+s, cy+s]], np.int32)
                        cv2.polylines(frame, [pts], True, shape.color, 5)
                else:
                    if shape.type == "circle":
            ...
            view_mode = "3D" if shape.is_3d else "2D"
            status = f"MODE: {view_mode} | SHAPE: {str(shape.type).upper()} | ROTATE: {'ON' if shape.is_rotating else 'OFF'}"
